...
> **凡例**
> - **[式]** 教材で用いる数式 / **[値]** 数値の根拠 / **[出典]** 一次／教科書文献
> - **[文献]** その数値・主張が引用先の**論文の抄録または本文に直接書かれている**ことを抄録レベルで確認したもの
> - **[教科書]** 数値・主張は教科書の慣用則／代表値で、教科書側を参照する必要があるもの
>   （Web 検索で単一の一次論文に帰着できないため [文献] とは別扱い）
> - **[教材導出]** 教材の模型眼計算・近似で**本教材が算出**した値（引用文献は理論枠組みのみ提供）
>
...
- **Φ_lens ≈ 8.4 + 14.0 − 8.4 × 14.0 × 0.00254 ≈ 22.1 D** **[教材導出]**

**この約 22 D は無調節状態（distance vision）の値**。Gullstrand No.1 模型眼の relaxed 水晶体 19.11 D、
Le Grand 完全模型眼の 21.78 D（**[教科書]** Le Grand 1980 / Atchison & Smith 該当章。
具体的数値は教科書側を参照）とほぼ同じオーダー。
本教材では調節（毛様体筋による水晶体形状変化）は考慮しない。

### 角膜屈折力（参考）

Φ_cornea = (1.336 − 1.0) / 0.0078 ≈ **+43.1 D** **[教材導出]**。
臨床通説の「角膜屈折力 約 43 D」**[教科書]**（Atchison & Smith 該当章）と一致 ✓。

### 全眼屈折力

3 屈折面の薄+厚レンズ系として近軸計算した全屈折力は約 60 D **[教材導出]**
（臨床通説 58〜60 D **[教科書]**：Atchison & Smith / Bennett & Rabbetts）と一致。
...
| 条件 | 計算値 | 臨床通説 | 出典 |
|---|---|---|---|
| 眼軸 24→25 mm | **−2.62 D/mm** **[教材導出]** | 約 −2.5〜−3.0 D/mm（教科書的範囲）**[教科書]** | Atchison & Smith 模型眼章 |
| 角膜 7.8→7.4 mm（半径 −0.4 mm） | 約 +3 D の近視化 **[教材導出]** | 約 1 D / 0.1 mm（臨床慣用則）**[教科書]** | Bennett & Rabbetts 屈折異常章 |
| 正常眼（24 mm, 7.8 mm） | −0.74 D（ほぼ正視）**[教材導出]** | 正視 | spec で校正可 |
...
| 条件 | 期待挙動 | 出所 | 出典 |
|---|---|---|---|
| 強・弱半径を等しく | 2 焦線が一致 → 円柱 0 | [教科書] | 任意の乱視光学教科書（自明） |
| 半径差 0.4 mm | 円柱約 2.5 D | [教材導出] | Bennett & Rabbetts 乱視章の枠組み |
| 軸角度回転 | 2 焦線が同期して回転 | [教科書] | パワークロス慣行（教科書） |
...
報告されている **[文献]**。これを線形近似して教材本文では「**1° の軸ずれで cylinder cancellation が約 3.3 % 失われる**」
（10° で 33 % ≈ 1/3）と表現している **[教材導出]**。これは小角近似での目安であり、大きな角度では
...
| 条件 | 期待 | 出所 | 出典 |
|---|---|---|---|
| IOL 軸 = 角膜軸 + 90° | 残余円柱 ≈ 0 | [教材導出] | Power Vector 合成（理論枠組: Thibos 1997 / Holladay 1992 / Alpins 1993） |
| ±10° 軸ずれ | 残余円柱 ≈ C × 1/3 | [文献] | Felipe 2011 抄録の原文表現 |
| 1° 軸ずれ ≈ 3.3 % 効果損失 | 教材本文の表現 | [教材導出] | Felipe "±10° = 1/3" の線形近似 |
| IOL 円柱が 0 | 角膜乱視がそのまま残る | [文献] | Power Vector 合成（自明） |
...
- **Felipe A, Artigas JM, Díez-Ajenjo A, García-Domene C, Alcocer P.** Residual astigmatism produced by toric intraocular lens rotation. *J Cataract Refract Surg* 2011;37(10):1895-901. PMID 21865007. — 軸回転と残余乱視の定量。
...
教材で扱うのは下表の **4 プリセット**のみ。媒質と水晶体有無の自由な組合せ
（例: 無水晶体＋オイル）は **文献的裏付けが限られる**ため UI では選択肢に含めない。

| プリセット | 媒質 n | 水晶体 | 屈折変化 |
|---|---|---|---|
| 標準眼 | 1.336 | あり | 0（基準） |
| シリコンオイル充填 | 1.40 | あり | **遠視化**（Φ_post 低下） |
| ガス充填 | ≈ 1.0 | あり | **強い近視化**（Φ_post 大幅増加） |
| 無水晶体眼 | 1.336 | なし | **強い遠視化**（≈22 D の水晶体パワー喪失） |

### 数値根拠

| 条件 | 教材の計算値 | 文献の臨床値 | 出所 | 出典 |
|---|---|---|---|---|
| シリコンオイル充填（有水晶体・模型眼計算） | **+7.91 D 遠視化** | — | [教材導出] | 本教材の模型眼への代数解 |
| シリコンオイル充填（pseudophakic, 臨床） | — | 平均 **+5.69 D**（範囲 +2.88〜+8.38, SD ±1.71） | [文献] | Hotta et al. 2005 抄録 |
| シリコンオイル充填（phakic, 臨床） | — | 遠視化 | [文献] | Smith RC et al. 1990 抄録 |
| ガス完全充填 | 強い近視化 | 完全充填中は視機能ほぼ消失 | 計算=[教材導出]<br>所見=[文献] | 教材計算＋一般臨床所見 |
| 無水晶体眼（水晶体除去のみ） | 強い遠視化 | 強い遠視化（aphakia の古典所見） | 計算=[教材導出]<br>所見=[文献] | Atchison & Smith aphakia 章 |

> ⚠️ **教材の +7.91 D は本コードによる模型眼の代数解**で、文献値（pseudophakic 臨床 +5.69 D 等）とは
> 直接の比較対象ではない。臨床の絶対値は IOL 形状・水晶体状態・瞳孔径・術後体位に依存する。
>
> ⚠️ **無水晶体＋オイル等の自由組合せは UI から除外**: 過去版では「無水晶体＋シリコンオイル
> → 近視化に逆転」を提示していたが、(水晶体有無 × 媒質) の任意組合せは文献蓄積が限られ、
> 教材の代数解と臨床値の対応も検証しきれない。Phase 4 は臨床的に頻出する 4 プリセットに限定する。
>
...
- **Smith RC, Smith GT, Wong D.** Refractive changes in silicone filled eyes. *Eye* 1990;4(Pt 1):230-4. PMID 2323475. — phakic 眼でのシリコンオイル充填による遠視化を臨床的に示した古典（aphakic 症例も論文に含まれるが、本教材はその組合せを UI で扱わない）。
...
### 屈折値 → Zernike 係数 の換算 **[教材導出]**

OSA 正規化（Thibos 2002）の厳密係数を、瞳半径 [m] / 屈折値 [D] の単位系で再表記したもの:

- **c₂⁰（defocus）= −(D · R_p²) / (4√3)**（R_p = 瞳半径 [m]、D = 屈折値 [D]）
- **c₂² (J₀)、c₂⁻² (J₄₅)** は Power Vector の J 成分から同様に換算（係数 √6/2 等）。

→ 詳細式は `src/optics/psf.ts` 内に検証コメント付き。
規格本体は **[文献]**（Thibos 2002 / ANSI Z80.28）、本教材で扱う単位系での再表記は **[教材導出]**。

### 視標と視角

- ランドルト環: 視力 **1.0** で **ギャップ 1 分角 / 環全体 5 分角**（国際標準 ISO 8596）**[文献]**。
...
- デフォーカス 0 で **回折限界 = Airy パターン**（中心強度、第一暗環半径 1.22·λ·f/D）**[教科書]**（Born & Wolf 該当章）。
...
- **[文献] / [教科書] / [教材導出] ラベル**: 各数値・主張が「論文の抄録／本文に直接書かれている」
  「教科書の慣用則／代表値で単一論文に帰着できない」「教材の模型眼計算・近似で算出した」
  のどれに当たるかを区別するため、2026-06 にラベル分離を行った（読者への透明性確保のため）。
- **[文献] ラベルの抄録レベル一致確認（2026-06-12 実施）**:
  - **Smith RC 1990 (PMID 2323475)** ✅ 抄録に "myopic shift in aphakic eyes / hypermetropic shift
    in phakic eyes" の明記を確認（Nature DOI:10.1038/eye.1990.32）
  - **Hotta 2005** ✅ 抄録に "+5.69 ± 1.71 D hyperopic shift in pseudophakic eyes" を確認
  - **Felipe 2011** ✅ 内容（±10° 回転で残余円柱 ≈ 1/3）を抄録で確認。
    ただし **PMID 22440449 → 21865007 に修正済**。22440449 は別論文（Hayashi et al.
    "Residual astigmatism after toric IOL implantation"）であった
  - **Thibos 1997 / 2002, Holladay 1992, Alpins 1993, Stefansson 2009, Charman 1991,
    Salmon 2006** — 書誌は確認済み。具体数値は教材本文では使用しておらず（理論枠組みのみ参照）
    抄録レベル数値一致は該当なし。
- **[教科書] ラベルとした項目**: 以下は単一の一次論文に帰着できず教科書の慣用則として扱う:
  軸長 −2.5〜−3.0 D/mm、角膜 1 D / 0.1 mm、角膜屈折力 43 D、全眼 58〜60 D、
  Le Grand 水晶体 21.78 D、Airy 1.22·λ·f/D。具体的数値は教科書側の該当章で確認のこと。
- **ISO 8596 / ANSI Z80.28 / Thibos 2002 (規格本体)** は規格文書を直接参照しており [文献] とする。
